          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">Strategic Diagnostic</span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <CardBody className="p-10 flex flex-col gap-8">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest opacity-60">Avg {isTOEFL ? "Score" : "Band"}</p>
                  <h3 className={`text-6xl font-black ${theme.text} tracking-tighter`}>{averages.band}</h3>
                </div>
          ...
          <Card className="border border-border/40 rounded-[40px] bg-card/50 backdrop-blur-sm overflow-hidden shadow-sm hover:shadow-lg transition-all duration-700">
            <CardBody className="p-10 flex flex-col gap-8">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest opacity-60">Tests Processed</p>
                  <h3 className="text-6xl font-black tracking-tighter">{averages.tests}</h3>
                </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-border/10">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  Diagnostic Frequency: <span className="text-foreground">Optimal Spectrum</span>
                </p>
              </div>
            </CardBody>
          </Card>
            ...
            <CardBody className="p-10 flex flex-col justify-between h-full gap-10 relative z-10">
              <div className="space-y-2">
                <div className={`flex items-center gap-2 ${theme.text}`}>
                  <Award size={18} />
                  <span className="text-[10px] font-black uppercase tracking-[0.2em]">Goal Optimization</span>
                </div>
                <p className="text-lg font-black tracking-tight uppercase">
                  Target {isTOEFL ? "Score" : "Band"}: <span className="text-foreground ml-2">{thresholdBand}{!isTOEFL && ".0"}+</span>
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {parseFloat(averages.band) >= thresholdBand && (
                  <p className={`text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-2 ${theme.text}`}>
              ...
              <CardBody className="p-10 flex flex-col gap-10">
                ...
                {learningPathError && (
                  <div className="flex items-start gap-4 p-6 rounded-3xl border border-destructive/20 bg-destructive/5">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-sm text-muted-foreground font-medium leading-relaxed">{learningPathError}</p>
                      <Link
                ...
                >
                  {loading ? (
                    ...
                    <><Sparkles className="mr-3 size-5" /> Execute Assessment Protocol</>
                  )}
                </Button>
              </CardBody>
            </Card>
                ...
                <CardBody className="p-10 space-y-10">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-black uppercase tracking-tight flex items-center gap-3">
                      <BarChart2 className="text-primary" size={20} /> Performance Trend
                    </h3>
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Last 7 Cycles</span>
                  </div>
                  <div className="flex items-end gap-3 h-32">
                    {chartData.map((item, i) => {
                      const h = Math.max(15, (parseFloat(String(item.overallBand)) / maxScore) * 100);
                      return (
                        <div key={item.id || i} className="flex-1 flex flex-col items-center gap-4 group">
                          <div className="w-full relative flex flex-col items-center">
                            ...
                            <div className="w-full bg-muted/30 rounded-full h-32 flex flex-col justify-end p-1 border border-border/10">
                              <motion.div
                                ...
                                initial={{ height: 0 }}
                                animate={{ height: `${h}%` }}
                                transition={{ delay: 0.1 * i, duration: 1, ease: "circOut" }}
                              />
                            </div>
                          </div>
              ...
              <CardBody className="p-10 space-y-8">
                ...
                {loadingStats ? (
                  <div className="flex flex-col items-center justify-center h-96 gap-4">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground animate-pulse">Syncing Archive...</p>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-96 text-center gap-6">
                    ...
                    <div className="space-y-2">
                      <p className="font-black uppercase tracking-widest text-foreground">Archive Empty</p>
                      <p className="text-sm text-muted-foreground max-w-xs mx-auto font-medium">
                        Initiate your first diagnostic protocol to populate the performance archive.
                      </p>
                    </div>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-center gap-8 flex-1 w-full sm:w-auto">
                          ...
                          <div className={`w-20 h-20 rounded-3xl ${parseFloat(String(item.overallBand)) >= thresholdBand ? 'bg-emerald-500/10 text-emerald-600' : 'bg-primary/10 text-primary'} flex flex-col items-center justify-center shrink-0 border border-current/10 shadow-sm relative group-hover:scale-105 transition-transform duration-500`}>
                            ...
                            <span className="text-[7px] font-black uppercase tracking-[0.2em] mt-1 opacity-60">Result</span>
                          </div>
