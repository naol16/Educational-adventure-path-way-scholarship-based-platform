...
export const useOnboarding = () => {
  ...
  // now accepts an optional data object from a custom form
  const handleStage3 = async (data: Record<string, unknown> = {}) => {
    setLoading(true);
    try {
      // merge any provided data with extracted defaults if available
      const basePayload =
        user?.role === "student"
          ? {
              calculatedGpa:
                (extractedData as StudentExtractedData)?.gpa || 3.5,
              academicHistory:
                (extractedData as StudentExtractedData)?.academic_history || [],
            }
          : {
              bio: (extractedData as CounselorExtractedData)?.bio || "",
              areasOfExpertise:
                (extractedData as CounselorExtractedData)?.areas_of_expertise ||
                [],
              yearsOfExperience:
                (extractedData as CounselorExtractedData)
