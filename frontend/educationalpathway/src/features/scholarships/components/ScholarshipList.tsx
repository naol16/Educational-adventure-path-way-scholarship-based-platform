...
export const ScholarshipList = ({ filters, activeTab, onPageChange }: ScholarshipListProps) => {
  const swrKey = useMemo(() => {
    let baseUrl = activeTab === 'explore' ? '/scholarships' : 
                   activeTab === 'matched' ? '/scholarships/match' : 
  ...
  const pagination = useMemo(() => {
    if ((activeTab === 'explore' || activeTab === 'matched') && rawData?.pagination) {
  ...
  const scholarships = useMemo<Scholarship[]>(() => {
    ...
    if (activeTab === 'saved') {
      return trackedItems
        .filter((item: any) => ['NOT_STARTED', 'WATCHING'].includes(item.status))
        .map((item: any) => ({
          ...item.scholarship,
    ...
    if (activeTab === 'applied') {
      return trackedItems
        .filter((item: any) => ['APPLIED', 'SUBMITTED', 'ACCEPTED'].includes(item.status))
        .map((item: any) => ({
          ...item.scholarship,
          tracking: { id: item.id, status: item.status }
        }));
    ...
    <div className="space-y-6">
      ...
      {loading ? (
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-10">
              <Button
                ...
                className="h-12 w-12 rounded-xl border-border/40 hover:bg-muted"
              >
                <ChevronLeft size={20} />
              </Button>
              ...
              <div className="flex items-center gap-2 px-4">
                {[...Array(pagination.totalPages)].map((_, i) => {
                  ...
                  if (
                    ...
                    return <span key={pageNum} className="text-muted-foreground/30 font-black">...</span>;
                  }
                  return null;
                })}
              </div>
        ...
        <div className="text-center py-24 bg-muted/20 rounded-xl border-2 border-dashed border-border/50 px-4">
          ...
          <h3 className="font-bold text-lg mb-1">No Recommended Scholarships</h3>
