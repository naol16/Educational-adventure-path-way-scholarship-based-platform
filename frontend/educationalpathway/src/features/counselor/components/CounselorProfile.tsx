...
// @ts-expect-error - CSS module type declaration not available
import "react-international-phone/style.css";
import { useGeoData } from "../../student/hooks/useGeoData";
import {
  FIELDS_OF_STUDY,
  FIELDS_OF_STUDY_GROUPED,
} from "../../student/constants/fields-of-study";
import { useAuth } from "@/providers/auth-context";

const COUNSELOR_POSITIONS = [
  {
    value: "Higher Education Consultant",
    label: "Higher Education Consultant",
  },
...
const AREAS_OF_EXPERTISE = [
  {
    value: "Full Scholarships (PhD/Masters)",
    label: "Full Scholarships (PhD/Masters)",
  },
  { value: "Undergraduate Admissions", label: "Undergraduate Admissions" },
  {
    value: "SOP & Personal Statement Review",
    label: "SOP & Personal Statement Review",
  },
  ...
  {
    value: "Research Proposal Development",
    label: "Research Proposal Development",
  },
    ...
    {option.flag &&
      (option.flag.endsWith(".svg") || option.flag.endsWith(".png") ? (
        <Image
          ...
          width={20}
          height={12}
          className="w-5 h-3 object-cover border border-border"
        />
      ) : (
...
export const CounselorProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        ["fieldsOfStudy", "areasOfExpertise", "consultationModes"].forEach(
          (field) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      payload.yearsOfExperience = Math.max(
        0,
        Number(payload.yearsOfExperience) || 0,
      );
      payload.sessionDuration = Math.max(
        1,
        Number(payload.sessionDuration) || 60,
      );
  ...
  const handleFileChange = (
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      toast.success(
        `${field.replace("Url", "").replace("profileImageUrl", "Photo")} selected`,
      );
    }
  };
  ...
  const updateConsultationMode = (mode: string) => {
    const current = profile!.consultationModes
      ? typeof profile!.consultationModes === "string"
        ? JSON.parse(profile!.consultationModes)
  ...
  const isPendingApproval =
    profile.isOnboarded && profile.verificationStatus === "pending";
          ...
          <div className="p-10 text-center space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-muted-foreground leading-relaxed">
                Thank you for completing your profile! Our team is currently
                reviewing your details. This usually takes{" "}
            ...
            >
              <div
                className={`h-10 w-10 rounded-xl flex items-center justify-center transition-all duration-300 ${
                  isActive
                    ? "primary-gradient text-white shadow-lg shadow-primary/30 ring-4 ring-primary/10"
                    : isCompleted
              ...
              <span
                className={`text-[10px] font-black uppercase tracking-widest hidden md:block ${isActive ? "text-primary" : "text-muted-foreground"}`}
              >
                {step.title}
              </span>
            </div>
                  ...
                  <div className="space-y-10">
                    <div className="space-y-4">
                      ...
                      <textarea
                        ...
                        className="w-full min-h-40 bg-muted/30 border border-border rounded-3xl p-6 text-sm focus:ring-2 focus:ring-primary/20 transition-all outline-none leading-relaxed font-medium"
                        placeholder="Explain your background, achievements, and how you can help students..."
                        required
                      />
                      <p className="text-[10px] text-muted-foreground ml-2 font-bold uppercase tracking-widest opacity-60">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-border/50">
                            ...
                            profile.countryOfResidence
                              ? {
                                  ...
                                  flag:
                                    countries.find(
                                      (c) =>
                                        c.name === profile.countryOfResidence,
                                    )?.flag || "",
                                }
                              : null
                          }
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          isDisabled={
                            !profile.countryOfResidence || loadingCities
                          }
                          onChange={(
                    ...
                    <div className="pt-12 border-t border-border flex justify-end">
                      <Button
                        onClick={nextStep}
                        className="h-14 px-12 gap-2 primary-gradient text-white rounded-2xl font-black uppercase tracking-widest text-xs"
                      >
                ...
                <div className="space-y-12">
                          ...
                          value={
                            profile.universityName
                              ? {
                                  ...
                                  label: profile.universityName,
                                }
                              : null
                          }
                          isDisabled={
                            !profile.countryOfResidence || loadingUniversities
                          }
                          onChange={(
                            val: SingleValue<{ value: string; label: string }>,
                          ) =>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          value={FIELDS_OF_STUDY.filter((f) => {
                            const currentFields = profile.fieldsOfStudy
                              ? Array.isArray(profile.fieldsOfStudy)
                                ? profile.fieldsOfStudy
                                : typeof profile.fieldsOfStudy === "string"
                                  ? JSON.parse(profile.fieldsOfStudy)
                                  : []
                              : [];
                          ...
                          onChange={(
                            ...
                            setProfile({
                              ...profile,
                              fieldsOfStudy: val.map((v) => v.value),
                            })
                          }
                          classNamePrefix="react-select"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          value={
                            COUNSELOR_POSITIONS.find(
                              (p) => p.value === profile.currentPosition,
                            ) ||
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          value={AREAS_OF_EXPERTISE.filter((a) => {
                            const currentAreas = profile.areasOfExpertise
                              ? Array.isArray(profile.areasOfExpertise)
                                ? profile.areasOfExpertise
                                : typeof profile.areasOfExpertise === "string"
                                  ? JSON.parse(profile.areasOfExpertise)
                                  : []
                              : [];
                          ...
                          onChange={(
                            ...
                            setProfile({
                              ...profile,
                              areasOfExpertise: val.map((v) => v.value),
                            })
                          }
                          classNamePrefix="react-select"
                  ...
                  <div className="pt-12 border-t border-border flex justify-between">
                    <Button
                      variant="outline"
                      onClick={prevStep}
                      className="h-14 px-10 gap-2 border-border font-black uppercase tracking-widest text-xs rounded-2xl"
                    >
                      <ChevronLeft size={16} /> Previous
                    </Button>
                    <Button
                      onClick={nextStep}
                      className="h-14 px-12 gap-2 primary-gradient text-white font-black uppercase tracking-widest text-xs rounded-2xl"
                    >
                      Next Step <ChevronRight size={16} />
                    </Button>
                  </div>
              ...
              {currentStep === 3 && (
                <div className="space-y-12">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-muted-foreground font-medium">
                      Set your consulting fees and preferred consultation
                      channels.
                    </p>
                  </div>
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              className="h-14 bg-muted/30 border-border rounded-2xl"
                            />
                          </div>
                          <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-2">
              ...
              {currentStep === 4 && (
                <div className="space-y-12">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-muted-foreground font-medium">
                      Upload your documents to complete the verification
                      process.
                    </p>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <div className="relative group overflow-hidden rounded-2xl aspect-square border-2 border-dashed border-border flex flex-col items-center justify-center hover:border-primary/50 transition-all bg-muted/20">
                          ...
                          <input
                            type="file"
                            onChange={(e) =>
                              handleFileChange("profileImageUrl", e)
                            }
                      ...
                      <div className="space-y-6">
                        <div className="space-y-3">
                          ...
                          <div
                            className={`p-6 border-2 border-dashed rounded-2xl flex items-center justify-between transition-all relative ${profile.cvUrl ? "border-emerald-500 bg-emerald-500/5" : "border-border hover:border-primary bg-muted/20"}`}
                          >
                            <div className="flex items-center gap-4">
                              <div
                                className={`p-3 rounded-lg ${profile.cvUrl ? "bg-emerald-500/20 text-emerald-500" : "bg-primary/10 text-primary"}`}
                              >
                                <FileText size={20} />
                              </div>
                              <div>
                                <p className="text-xs font-black uppercase tracking-tight">
                                  {profile.cvUrl ? "Resumé Ready" : "Upload CV"}
                                </p>
                        ...
                        <div className="space-y-3">
                          ...
                          <div
                            className={`p-6 border-2 border-dashed rounded-2xl flex items-center justify-between transition-all relative ${profile.certificateUrls ? "border-emerald-500 bg-emerald-500/5" : "border-border hover:border-primary bg-muted/20"}`}
                          >
                            <div className="flex items-center gap-4">
                              <div
                                className={`p-3 rounded-lg ${profile.certificateUrls ? "bg-emerald-500/20 text-emerald-500" : "bg-primary/10 text-primary"}`}
                              >
                                <GraduationCap size={20} />
                              </div>
                              <div>
                            ...
                            <input
                              type="file"
                              onChange={(e) =>
                                handleFileChange("certificateUrls", e)
                              }
                              className="absolute inset-0 opacity-0 cursor-pointer"
                            />
