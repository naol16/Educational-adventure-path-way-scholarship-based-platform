...
export function ActiveCallInterface({ assistantConfig, interviewId, country, onCallEnd }: ActiveCallInterfaceProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[600px] gap-8 p-4">
      {/* Simulation Header */}
      <div className="w-full max-w-2xl flex items-center justify-between px-6 py-4 bg-muted/30 rounded-2xl border border-border/50">
      ...
      <Card className="w-full max-w-2xl border-none bg-background rounded-2xl overflow-hidden relative">
        ...
        <CardBody className="p-12 sm:p-20 flex flex-col items-center gap-12 relative z-10">
          <div className="relative cursor-pointer group" onClick={() => {
            ...
            <div className={`size-40 rounded-full bg-background border-4 ${isRecording ? 'border-red-500/20' : 'border-primary/20'} flex items-center justify-center relative z-10 overflow-hidden ${callStatus === 'connecting' || isThinking ? 'animate-pulse' : ''}`}>
               {isRecording ? (
          ...
          <div className="w-full p-6 bg-primary/5 rounded-2xl border border-primary/10 text-center min-h-[100px] flex items-center justify-center">
        ...
        <div className="bg-muted/10 border-t border-border/40 p-8 flex items-center justify-center gap-6">
          ...
          <Button
            variant="destructive"
            size="lg"
            className="h-16 px-10 rounded-lg font-black uppercase tracking-widest gap-3 hover:scale-105 transition-transform"
