...
export function PostInterviewAnalytics({ interviewId, onRestart }: PostInterviewAnalyticsProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-32 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      {/* Score Header */}
      <div className="relative rounded-2xl overflow-hidden bg-background border-4 border-primary/20 p-12 lg:p-20 text-center">
          ...
          {rubricBreakdown && Object.keys(rubricBreakdown).length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 pt-4">
              {Object.entries(rubricBreakdown).map(([key, value]) => (
                <div key={key} className="rounded-2xl border border-primary/15 bg-primary/5 px-3 py-3 text-center">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="space-y-6">
           ...
           <div className="grid gap-3">
             {(evaluation?.focus_areas || []).map((area: string, i: number) => (
               <div key={i} className="p-4 rounded-2xl bg-primary/5 border border-primary/10 flex items-center gap-3">
        ...
        {/* Simulation Summary */}
        <div className="space-y-6">
           <Card className="border-none bg-muted/20 rounded-2xl h-full">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-12 border-t border-border">
         ...
         <Button onClick={() => window.location.href = '/dashboard'} size="xl" className="w-full sm:w-auto px-10 h-16 rounded-lg primary-gradient gap-3 hover:scale-105 transition-transform font-black uppercase tracking-widest">
