...
export class AIChatController {
  static async askGeneral(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const response = await AIChatService.askGeneralAssistant(
        message,
        sessionId,
        userId,
      );
    ...
    try {
      ...
      if (!message || !sessionId || !id) {
        return res.status(400).json({
          error: "Message, sessionId, and scholarship id are required",
        });
      }

      const response = await AIChatService.askScholarshipAssistant(
        Number(id),
        message,
        sessionId,
        userId,
