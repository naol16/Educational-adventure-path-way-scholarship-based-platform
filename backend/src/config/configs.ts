...
function setConfigs() {
  return {
    ...
    JWT_ACCESS_EXPIRATION:
      process.env.JWT_ACCESS_EXPIRATION || process.env.JWT_EXPIRES_IN || "1d",
    ...
    SMTP_FROM:
      ...
      `"Educational Pathway" <noreply@edu-pathway.com>`,
    ...
    QUARTERLY_PRICE_IN_ETB: parseFloat(
      process.env.QUARTERLY_PRICE_IN_ETB || "0",
    ),
    ANNUAL_PRICE_IN_ETB: parseFloat(process.env.ANNUAL_PRICE_IN_ETB || "0"),
    SEMIANNUAL_PRICE_IN_ETB: parseFloat(
      process.env.SEMIANNUAL_PRICE_IN_ETB || "0",
    ),
