...
class _SessionDetailScreenState extends ConsumerState<SessionDetailScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            if (booking.status == 'confirmed')
              _buildJoinButton(context, booking),
  ...
  Widget _buildStatusHeader(BuildContext context, CounselorBooking booking) {
    ...
    switch (booking.status) {
      case 'confirmed':
        statusColor = DesignSystem.success(context);
        statusIcon = LucideIcons.checkCircle;
        break;
      case 'started':
        statusColor = DesignSystem.primary(context);
        statusIcon = LucideIcons.video;
        break;
      case 'awaiting_confirmation':
        ...
        break;
      case 'completed':
        statusColor = DesignSystem.info(context);
        statusIcon = LucideIcons.checkSquare;
        break;
      case 'cancelled':
        statusColor = DesignSystem.error(context);
  ...
  Widget _buildStudentCard(BuildContext context, StudentSummary? student) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => StudentProgressDetailScreen(student: student),
        ),
      ),
      ...
      child: Column(
        children: [
          _timeRow(
            ...
            'Date',
            DateFormat('EEEE, MMM d, yyyy').format(slot.startTime),
          ),
          Padding(
            padding: const EdgeInsets.symmetric(vertical: 12),
            child: Divider(height: 1, color: DesignSystem.glassBorder(context)),
          ),
          _timeRow(
            context,
            LucideIcons.clock,
    ...
    return Row(
      children: [
        Container(
          ...
          child: Icon(icon, size: 20, color: DesignSystem.primary(context)),
        ),
        const SizedBox(width: 16),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: _isUpdating
                  ? null
                  : () => _updateStatus('awaiting_confirmation'),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              style: OutlinedButton.styleFrom(
                foregroundColor: DesignSystem.primary(context),
                side: BorderSide(color: DesignSystem.primary(context)),
              ...
              label: Text(
                isStarted ? 'RE-JOIN MEETING' : 'JOIN MEETING',
                style: GoogleFonts.plusJakartaSans(
                  fontWeight: FontWeight.w800,
                  fontSize: 16,
                ),
        ...
        if (booking.status == 'awaiting_confirmation')
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: DesignSystem.warning(context).withValues(alpha: 0.2),
              ),
            ),
            child: Row(
              children: [
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            hintText: 'Add private notes or student feedback...',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              final ok = await ref
                  .read(counselorAppServiceProvider)
                  .updateBookingNotes(widget.booking.id, controller.text);
              if (ok) {
                ref.invalidate(counselorUpcomingBookingsProvider);
